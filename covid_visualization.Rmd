...
```{r}
data = read_excel("./data/nyc_covid.xlsx")
```
...
The graph below shows the number of new cases recorded each day from February 29, 2020 to May 25, 2020. 

```{r}
data %>% 
  plot_ly(x = ~Date, y = ~`Case Count`, type = "bar") %>% 
  layout(
...
Now we can visualize the cumulative number of cases, again from February 29, 2020 to May 25, 2020.
...
```{r}
by_zip = read_excel("./data/nyc_covid_by_zcta.xlsx") %>% 
...
Testing the correlation coefficient, we obtain a Rho of -0.43 and an associated p-value of $2.7*10^{-9}$. This indicates that case rate is negatively correlated with proportion of white householders in the ZCTA, and the correlation is statistically significant. A higher case rate is associated with a lower proportion of white householders.    

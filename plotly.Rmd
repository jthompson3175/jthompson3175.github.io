...
rest_clean %>% 
  ...
  group_by(boro, year) %>% 
  summarize(
    proportion_verm = sum(contains_vermin)/n()*100
  ) %>% 
  plot_ly(x = ~year, y = ~proportion_verm, type = "scatter", mode = "lines+markers",
    color = ~boro, alpha = 0.5) %>% 
    layout(
    title = "Percentage of Violation Descriptions Mentioning Vermin by Borough Over Time",
    xaxis = list(title = "Year"),
    yaxis = list(title = "Percentage of Violation Descriptions Mentioning Vermin")
  )
```
